...
class CI_Session {
	...
	function sess_run()
	{
		...
		if ($this->CI->config->item('sess_use_database') === TRUE AND $this->CI->config->item('sess_table_name') != '')
		{
			...
			$this->siteDBC = $this->CI->load->database('st_site', TRUE);
		}
	...
	{	
		...
		if ($this->use_database === TRUE)
		{
			$this->siteDBC->where('session_id', $session['session_id']);
					
			if ($this->CI->config->item('sess_match_ip') == TRUE)
			{
				$this->siteDBC->where('ip_address', $session['ip_address']);
			}

			if ($this->CI->config->item('sess_match_useragent') == TRUE)
			{
				$this->siteDBC->where('user_agent', $session['user_agent']);
			}
			
			$query = $this->siteDBC->get($this->session_table);
			...
			else
			{
				...
				{
					$this->siteDBC->where('session_id', $session['session_id']);
					$this->siteDBC->delete($this->session_table);
					$this->sess_destroy();
					return FALSE;
				}
			}
	...
	{	
		...
		if ($this->use_database === TRUE)
		{
			$this->siteDBC->query($this->siteDBC->insert_string($this->session_table, $this->userdata));
		}
	...
	{	
		...
		if ($this->use_database === TRUE)
		{		
			$this->siteDBC->query($this->siteDBC->update_string($this->session_table, array('last_activity' => $this->now), array('session_id' => $this->userdata['session_id'])));
		}
	...
	function sess_gc()
	{
		...
		if ((rand() % 100) < $this->gc_probability)
		{
			...
			$this->siteDBC->where("last_activity < {$expire}");
			$this->siteDBC->delete($this->session_table);
